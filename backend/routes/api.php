...
Route::prefix('auth')->group(function () {
    ...
    Route::get('/verify-email/{id}/{hash}', [AuthController::class, 'verifyEmail'])->middleware('throttle:10,1')->name('verification.verify');
...
Route::middleware(['auth:sanctum', 'throttle:60,1'])->prefix('admin')->group(function () {
    ...
    Route::get('/users', [AdminController::class, 'users'])->middleware(CheckRole::class . ':admin');
    Route::get('/users/{user}', [AdminController::class, 'showUser'])->middleware(CheckRole::class . ':admin');
    Route::put('/users/{user}', [AdminController::class, 'updateUser'])->middleware(CheckRole::class . ':admin');
    Route::delete('/users/{user}', [AdminController::class, 'deleteUser'])->middleware(CheckRole::class . ':admin');

    Route::get('/products', [AdminController::class, 'products'])->middleware(CheckRole::class . ':admin');
    Route::put('/products/{product}', [AdminController::class, 'updateProduct'])->middleware(CheckRole::class . ':admin');
    Route::delete('/products/{product}', [AdminController::class, 'deleteProduct'])->middleware(CheckRole::class . ':admin');

    Route::get('/orders', [AdminController::class, 'orders'])->middleware(CheckRole::class . ':admin');
    Route::get('/orders/{order}', [AdminController::class, 'showOrder'])->middleware(CheckRole::class . ':admin');
    Route::put('/orders/{order}', [AdminController::class, 'updateOrder'])->middleware(CheckRole::class . ':admin');

    Route::get('/reports/sales', [AdminController::class, 'salesReport'])->middleware(CheckRole::class . ':admin');
    Route::get('/reports/top-products', [AdminController::class, 'topProducts'])->middleware(CheckRole::class . ':admin');
    Route::get('/reports/top-farmers', [AdminController::class, 'topFarmers'])->middleware(CheckRole::class . ':admin');
});
...
Route::middleware('throttle:60,1')->prefix('products')->group(function () {
...
Route::middleware(['auth:sanctum', 'throttle:60,1'])->prefix('cart')->group(function () {
...
Route::middleware(['auth:sanctum', 'throttle:60,1'])->prefix('orders')->group(function () {
...
Route::middleware('throttle:60,1')->prefix('payments')->group(function () {
    Route::post('/initialize', [App\Http\Controllers\Api\PaymentController::class, 'initialize'])->middleware('auth:sanctum');
    Route::post('/verify', [App\Http\Controllers\Api\PaymentController::class, 'verify'])->middleware('auth:sanctum');
...
Route::middleware('throttle:60,1')->prefix('products/{product}/reviews')->group(function () {
